...
public class LogisticsDocAction extends ActionSupport implements ServletRequestAware {
    ...
    private static Logger logger = LogManager.getLogger(LogisticsDocAction.class.getName());
    ...
    public String getDocumentSearchQuery() throws Exception {
        ...
        if (httpServletRequest.getSession(false).getAttribute(AppConstants.SES_USER_NAME) != null) {
            try {
                System.out.println("getDocumentSearchQuery start time::" + DateUtility.getInstance().getCurrentDB2Timestamp());
                execute();
                if (getCheck() == null) {
                    setCheck("1");
                ...
                LoggerUtility.log(logger, "Exception occurred in getDocumentSearchQuery method:: " + exception.getMessage(), Level.ERROR, exception.getCause());
                setResultType("error");
            }
        }
        System.out.println("TpoPartnerProfiles start time::" + DateUtility.getInstance().getCurrentDB2Timestamp());
        return getResultType();
    }
